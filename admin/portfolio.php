...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($action, ['create', 'edit'])) {
    ...
    $type = in_array(post('type'), ['web', 'mobile', 'desktop', 'system']) ? post('type') : 'web';
    ...
    $project_url = trim(post('project_url'));
    $featured_image = trim(post('featured_image'));
    $gallery_images = trim(post('gallery_images'));
    ...
    if (empty($errors)) {
        $safe = [];
        foreach (['title','slug','client','description','challenge','solution','results','technologies','project_url','featured_image','gallery_images'] as $f) {
            $safe[$f] = db_escape($$f);
        }
        ...
        if ($action === 'create') {
            db_query("INSERT INTO portfolio (title, slug, type, service_id, client, description, challenge, solution, results, technologies, project_url, featured_image, gallery_images, is_featured, status, created_at) 
                      VALUES ('{$safe['title']}', '{$safe['slug']}', '$type', $service_id, '{$safe['client']}', '{$safe['description']}', '{$safe['challenge']}', '{$safe['solution']}', '{$safe['results']}', '{$safe['technologies']}', '{$safe['project_url']}', '{$safe['featured_image']}', '{$safe['gallery_images']}', $is_featured, '$status', NOW())");
            log_activity('create', 'portfolio', db_insert_id(), 'Created project: ' . $title);
            set_flash('success', 'Project created.');
        } else {
            db_query("UPDATE portfolio SET title='{$safe['title']}', slug='{$safe['slug']}', type='$type', service_id=$service_id, client='{$safe['client']}', description='{$safe['description']}', challenge='{$safe['challenge']}', solution='{$safe['solution']}', results='{$safe['results']}', technologies='{$safe['technologies']}', project_url='{$safe['project_url']}', featured_image='{$safe['featured_image']}', gallery_images='{$safe['gallery_images']}', is_featured=$is_featured, status='$status', updated_at=NOW() WHERE id=$id");
...
$services = db_fetch_all("SELECT id, title FROM services ORDER BY title ASC");

// ---- CREATE/EDIT FORM ----
if (in_array($action, ['create', 'edit'])):
    $item = ['title'=>'','slug'=>'','type'=>'web','service_id'=>0,'client'=>'','description'=>'','challenge'=>'','solution'=>'','results'=>'','technologies'=>'','project_url'=>'','featured_image'=>'','gallery_images'=>'','is_featured'=>0,'status'=>'draft'];
    if ($action === 'edit' && $id > 0) {
    ...
    <form method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-black/5 dark:border-white/10 rounded-2xl p-6 space-y-4">
            <div>
                ...
                <input type="text" name="title" id="proj-title" value="<?php echo e($item['title']); ?>" required class="form-input">
            </div>
            <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="type" class="form-input">
                        <?php foreach (['web'=>'Web App','mobile'=>'Mobile App','desktop'=>'Desktop','system'=>'System'] as $k=>$v): ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="client" value="<?php echo e($item['client']); ?>" class="form-input">
                </div>
                <div>
                    <label class="form-label">Project URL</label>
                    <input type="url" name="project_url" value="<?php echo e($item['project_url']); ?>" class="form-input" placeholder="https://...">
                </div>
            </div>

            <div>
                <label class="form-label">Description</label>
                <textarea name="description" rows="4" class="form-input"><?php echo e($item['description']); ?></textarea>
            </div>
            <div>
                <label class="form-label">Challenge</label>
                <textarea name="challenge" rows="3" class="form-input"><?php echo e($item['challenge']); ?></textarea>
            </div>
            <div>
                <label class="form-label">Solution</label>
                <textarea name="solution" rows="3" class="form-input"><?php echo e($item['solution']); ?></textarea>
            </div>
            <div>
                <label class="form-label">Results (JSON)</label>
                <textarea name="results" rows="3" class="form-input font-mono text-sm" placeholder='[{"label":"Metric","value":"Result"}]'><?php echo e($item['results']); ?></textarea>
            </div>
            <div>
            ...
            <div>
                ...
                <textarea name="gallery_images" rows="2" class="form-input font-mono text-sm"><?php echo e($item['gallery_images']); ?></textarea>
            </div>
...
$items = db_fetch_all("SELECT p.*, s.title as service_name FROM portfolio p LEFT JOIN services s ON p.service_id=s.id $where ORDER BY p.created_at DESC LIMIT {$pagination['offset']}, {$pagination['per_page']}");
?>

<div class="flex flex-wrap items-center justify-between gap-4 mb-6">
    <div class="flex items-center gap-2">
        <?php foreach ([''=>'All', 'web'=>'Web', 'mobile'=>'Mobile', 'desktop'=>'Desktop', 'system'=>'System'] as $k=>$v): ?>
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-white/5">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                <?php endif; ?>
                                <div>
                                    <div class="font-semibold text-slate-700 dark:text-gray-200"><?php echo e(truncate($p['title'], 40)); ?></div>
                        ...
                        <td class="px-4 py-3 text-slate-500"><?php echo e($p['client'] ?: '—'); ?></td>
                        <td class="px-4 py-3">
